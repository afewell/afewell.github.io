...
class Sub:
    ...
    def _prepare(self):
        ...
        if self._recursive_contract_dirs:
            # also take root's recursive contracts
    ...
    def _prep_mod(self, mod: ModuleType, iface: str, bname: str):
        ...
        tmp_recursive_contracts = pop.contract.load_contract(
        ...
        if getattr(self._root, "_recursive_contracts", None):
            tmp_recursive_contracts.extend(
        ...
        recursive_contracts = []
        for contract in tmp_recursive_contracts:
            if contract not in recursive_contracts:
                recursive_contracts.append(contract)
        name = vret["name"]
