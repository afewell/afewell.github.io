...
async def search_raw(
    ...
    """
    Fetch one or more volumes from AWS. The return will be in the same format as what the boto3 api returns.
    ...
    ret = await hub.exec.boto3.client.ec2.describe_volumes(
        ctx, Filters=boto3_filter, VolumeIds=[resource_id] if resource_id else None
    )
