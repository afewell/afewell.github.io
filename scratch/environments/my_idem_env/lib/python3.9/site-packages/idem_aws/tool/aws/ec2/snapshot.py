...
async def search_raw(
    ...
    """
    Fetch one or more snapshots from AWS. The return will be in the same format as what the boto3 api returns.

    Args:
        resource_id(str, Optional): Resource ID of the snapshot.
    ...
    ret = await hub.exec.boto3.client.ec2.describe_snapshots(
        ctx,
        Filters=boto3_filter,
        SnapshotIds=[resource_id] if resource_id else None,
        OwnerIds=["self"],
    )
