...
async def update(
    ...
    """
    Update tags of AWS ElasticLoadBalancingv2 resources.

    Args:
        resource_id(str): AWS ElasticLoadBalancingv2 resource id
    ...
    if tags_to_remove:
        if not ctx.get("test", False):
            delete_ret = await hub.exec.boto3.client.elbv2.remove_tags(
                ctx,
    ...
    if tags_to_add:
        if not ctx.get("test", False):
            add_ret = await hub.exec.boto3.client.elbv2.add_tags(
                ctx,
                ResourceArns=[resource_id],
